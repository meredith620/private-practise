...
iptables -t nat -A OUTPUT -p tcp -m set ! --match-set china dst -j V2RAY # 代理网关本机
iptables -t nat -A PREROUTING -p tcp -m set ! --match-set china dst -j V2RAY  # 代理局域网设备 12345 端口（即 V2Ray）
