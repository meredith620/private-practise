# opkg install ip-tiny ipset iptables kmod-ipt-ipset kmod-ipt-tproxy libipset iptables-mod-tproxy 
...
# iptables -t mangle -A V2RAY -d 10.10.10.0/24 -p tcp -j RETURN # 直连局域网，避免 V2Ray 无法启动时无法连网关的 SSH，如果你配置的是其他网段（如 10.x.x.x 等），则修改成自己的
# iptables -t mangle -A V2RAY -d 10.10.20.0/24 -p udp ! --dport 53 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 的 
...
#iptables -t mangle -A V2RAY_MASK -d 10.10.10.0/24 -p tcp -j RETURN # 直连局域网
#iptables -t mangle -A V2RAY_MASK -d 10.10.20.0/24 -p udp ! --dport 53 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 的 DNS）
